...
public class HomeController {
    ...
    @RequestMapping("/complete/{val}")
    public String comTodo(@PathVariable("val") long val, Model model)
    {
        if(val == 0)
        {
            model.addAttribute("icon","check.png");
        }
        else
        {
            model.addAttribute("icon","circle.jpg");
        }
        return "image";
    }
}
